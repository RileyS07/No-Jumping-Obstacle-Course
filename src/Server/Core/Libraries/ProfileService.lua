			...
			loaded_data, key_info = CustomWriteQueueAsync(
				function() -- Callback
					if is_get_call == true then
						...
						if version ~= nil then
							local get_version_success, get_version_error_message = pcall(function()
								get_data, get_key_info = profile_store._global_data_store:GetVersionAsync(profile_key, version)
							end)
							if get_version_success == false and type(get_version_error_message) == "string" and string.find(get_version_error_message, "not valid") ~= nil then
